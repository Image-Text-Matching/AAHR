...
def encode_data(model, data_loader, log_step=10, logging=logger.info):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        images, full_imag, image_lengths, captions, clip_captions, lengths, ids, img_ids, repeat = data_i
        model.logger = val_logger

        # compute the embeddings
        img_emb, cap_emb = model.forward_emb(images, full_imag, captions, clip_captions, img_ids, lengths, image_lengths=image_lengths)
...
def evalrank(model_path, opt=None, tokenizer=None, model=None, split='dev', fold5=False, save_path=None, data_path=None):
    ...
    data_loader = image_caption.get_test_loader(data_path, split, tokenizer,
                                                clip_tokenizer, preprocess,
...
def encode_data_clip( data_loader, log_step=10, logging=logger.info):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        with torch.no_grad():
            clip_image_encoder = clip_encoder.get_clip_image_encoder()
            clip_img_emb = clip_image_encoder(full_imag)
        with torch.no_grad():
            clip_text_encoder = clip_encoder.get_clip_text_encoder()
            clip_cap_emb = clip_text_encoder(clip_captions)
        # # compute the embeddings
        # img_emb, cap_emb = model.forward_emb(images, full_imag, captions, clip_captions, img_ids, lengths,
        #                                      image_lengths=image_lengths)
        img_emb = clip_img_emb
        cap_emb = clip_cap_emb
    ...
    return img_embs, cap_embs
def evalrank_clip( opt=None, tokenizer=None, split='dev', fold5=False, save_path=None,
             data_path=None):
    clip = ClipEncoder()
    clip_tokenizer = clip.clip_tokenizer
    preprocess = clip.preprocess
    ...
    logger.info('Computing embeddings.')
    with torch.no_grad():
        img_embs, cap_embs = encode_data_clip(data_loader)
